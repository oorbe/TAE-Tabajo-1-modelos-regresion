...
Para obtener la base de datos final fue necesario hacer un análisis con las diferentes bases de datos que habian en el DANE, para desechar las variables que consideraramos poco relevantes para el desarrollo del problema y para tener en cuenta solamente las variables con las cuáles se podría predecir el número de hiojos en Colombia.

Se tomó en cuenta el modelo relacional con el que estaban construidas las bases de datos y se obtuvieron los siguientes resultados para cada tipo de tablas:
...
```{r, include=FALSE, echo=FALSE, warning=F, message=FALSE}
# load libraries
library(caret)
library(tidyverse)
library(dplyr)
...
```{r setup, include=FALSE, echo=FALSE , warning=F, message=FALSE}
# Directorio de trabajo 
# setwd("C:/Users/Sara/OneDrive/Documentos/Semestre 01-2021/TAE/Trabajo 1/Datos")
#setwd("~/9/TAE/Trabajo1 regresion")
setwd("D:/juanm/Documents/documentosImportantes/semestre 8/TAE/trabajos/TAE-Tabajo-1-modelos-regresion")
# Lectura de base de datos 
Datos <- read.csv("datos.csv",header = TRUE)
```

### Selección de Variables para un modelo KNN:

**knn es muy lento cuando se tienen muchas observaciones por que escanea la base de datos hisotorica para hacer predicciones, el paquete Caret ofrece algunas funciones pero sus resultados para esta base de datos son muy ineficientes por esto no se implementó**

**Caret::varImp** : cálculo de la importancia de la variable para modelos de regresión y clasificación

Para calcular la importancia de una variable primero ajustamos un modelo genérico knn() con un subconjunto de nuestra base de datos (Datos de entrenamiento).

El codigo se muestra pero no se corre por lo anteriormente mencionado.
...
# Selección de variables. Se configura para obtener el mejor modelo con 1 y con 2 variables izes=c(1,2)):
...
El paquete **caret R** proporciona herramientas para informar automáticamente sobre la relevancia e importancia de los atributos en los datos e incluso seleccionar las características más importantes. Se analizará la **Matriz de correlación** para identificar atributos altamente correlacionados y eliminarlos en cyo caso esta corrleción sea mayor de 0.75 ya que nos estaria indicando que posiblemente haya redundancia entre esas dos variables altamente correlacionadas si se dejan ambas en el modelo ya que una estaria relacionada con la otra positiva o negativamente segun sea el caso.
...
```{r}
kable(round(apply(Datos_num,2,sd),2),  booktabs = TRUE) %>%
  kable_styling(font_size = 18)
```

Esto nos permite analizar la variabilidad que hay presente en las variables numericas.
...
Con KNN es debido hacer normalización de los datos para que el algoritmo trabaje las mismas unidades internamente para seleccionar los k vecinos mas cercanos a una variable.
...
```{r}
kable(round(prop.table(table(datos_train$n)),2),  booktabs = TRUE) %>%
  kable_styling(font_size = 18)
```

```{r}
kable(round(prop.table(table(datos_test$n)),2),  booktabs = TRUE) %>%
  kable_styling(font_size = 18)
```

### Modelos de regresión KNN

Probamos un modelo de regresión KNN con diferentes K y asi evaluar con nuestro conjunto de entrenamientos y prueba el comportamiento con la medidas RMSE, se escoge aquel modelo cuya medida RMSE sea minima.

**K=1**

```{r}
# k=1 
inicio <- Sys.time() # utilzamos esta funcion para medir el tiempo
knnn <- knnreg(n~. ,data = datos_train, k=1) # knnreg realiza regesion knn
predict1 <- predict(knnn, newdata = datos_test) # se predicen los valores
fin <- Sys.time() 
fin-inicio # se otien el tiempo para entrenar y predecir
RMSE1 <- sqrt(mean((predict1-datos_test$n)^2)) # se calcula el RMSE
# Los siguientes chunks son los mismo pero solo varia k.
```
...
```{r}
saveRDS(knn20,'knn.rds') # se guardará el mejor modelo, teniendo en cuenta un menor RMSE
rm(knn20) # removemos el modelo de enviroment para cargarlo
knn20 <- readRDS('knn.rds') # se carga el modelo guardado
```

```{r}
comparacion_knn <- data.frame(RMSE=c(RMSE1,RMSE3,RMSE5,RMSE10,RMSE15,RMSE20,RMSE30), row.names  = c(paste0('k=',c(1,3,5,10,15,20,30))))

kable(comparacion_knn,  booktabs = TRUE) %>%
  kable_styling(font_size = 18)
```

```{r}
plot(y=c(RMSE1,RMSE3,RMSE5,RMSE10,RMSE15,RMSE20,RMSE30),x=c(1,3,5,10,15,20,30), type="o",
     xlab = 'k', ylab = 'RMSE', main = 'Elbow plot de modelo knn respecto al RMSE', lwd=2)
```

Una vez probada la modelación con knnreg() con todas las variables predictivas de nuestra base de datos se encontro que el menor RMSE para estos modelos de regresión KNN se tiene con k=20, a medidas que se aumenta k no se observa diferencia significativa en la mejora del RMSE, es decir de que este entregue predicciones mejores por lo cual utilizaremos este para entrenar y validar nuestros datos, asi mismo para usarlo como estimación en nuestra app web.
...
kable(p20*escala[1]+centro[1],  booktabs = TRUE) %>%
  kable_styling(font_size = 18)
```

Se obtubo una respuest cercana a la realidad, subestimando la predicción realizada pero aun asi con resultados satisfactorios. Aunque tambien se hizo la prueba con otras observaciones la calidad de la predicción no es perfecta. Se sugiere para futuros estudios optimizar las variables predictoras para una mejora del modelo.

### Función para encontrar los vecinos mas cercanos y promediar sus respuestas:

Se quiso utilizar el algoritmo utilizado en clase para realizar cross validation, Se comenta el procedimiento realizado debido a que la dimensión de los datos no nos permite tener un tiempo optimo para la realización del proceso. Por lo cual se descarta su aplicación para este problema; Ademas de que poseemos un conjunto amplio de observaciones nos permite fundamentar igualmente la omisión de este procedimiento.
...
Podemos observar que no existe mucha correlación con respecto a la variable número de hijos (variable n) sin embargo probaremos con algunos modelos lineales:

![](pairs%20de%20datos.png)
...
Viendo el R\^2 con una sola variable será de 5.96% pero usando todas maximo optiene un 18.8%

```{r}
kable(sumary$rsq,  booktabs = TRUE) %>%
...
```{r}
par(mfrow=c(2,2))
plot(sumary$rsq ,xlab=" Number of Variables ",ylab=" R^2", type="o", lwd=2)
plot(sumary$adjr2 ,xlab=" Number of Variables ",ylab=" R^2 adjus", type="o", lwd=2)
plot(sumary$rss ,xlab=" Number of Variables ",ylab=" RSS", type="o", lwd=2)
plot(sumary$bic ,xlab=" Number of Variables ",ylab="BIC", type="o", lwd=2)

```

Al parecer es en general es suficiente utilizar 10 variables para generalizar un buen modelo:

Forward and Backward y Selection: Se puede observar que se obtienen las mismas variables para un tamaño adecuado de 10 en todos los métodos de selección:

```{r, message=F , warning= F}
reg_forw_fit <- regsubsets(n~.,data = datos, nvmax=34, method = 'f')
reg_back_fit <- regsubsets(n~.,data = datos, nvmax=34, method = 'b')

kable(coef(reg_full_fit, 10),align = 'c',  booktabs = TRUE) %>%
  kable_styling(font_size = 15)
kable(coef(reg_forw_fit, 10),align = 'c',  booktabs = TRUE) %>%
  kable_styling(font_size = 15)
kable(coef(reg_back_fit, 10),align = 'c',  booktabs = TRUE) %>%
  kable_styling(font_size = 15)
```

Ahora vamos a realiza la selección de variables utilizando rich y lasso regresión:
...
Al parecer algunos coeficientes tienden a cero a medida que se mueven los valores del lambda: realizamos particionamiento de los datos en entrenamiento y prueba para realizar validación cruzada y encontrar el mejor lambda:
...
Planteamos el modelo y obtenermos el RMSE de prueba:

```{r}
ridge.mod =glmnet (x[train,],y[train],alpha =0, lambda =grid ,
thresh =1e-12)
ridge.pred=predict(ridge.mod ,s=4, newx=x[test ,])
kable(mean(( ridge.pred -y.test)^2),align = 'c',  booktabs = TRUE) %>%
  kable_styling(font_size = 15)

```

Se obtiene el mejor lambda utilizando validación cruzada y luego se calcula la predicción con el:
...
kable(bestlam ,align = 'c',  booktabs = TRUE) %>%
  kable_styling(font_size = 15) # el mejor valor e lambda
ridge.pred=predict (ridge.mod ,s=bestlam ,newx=x[test ,])
kable(sqrt(mean(( ridge.pred -y.test)^2)),align = 'c',  booktabs = TRUE) %>%
  kable_styling(font_size = 15) # aquie tenemos su RMSE
```

Seleccionamos las varaibles con un criterio de que el coeficientes se mayor a 0.0001 o que sea menor que cero y tenemos la siguientes variables:

```{r}
out=glmnet (x,y,alpha =0)
rigge.coef <- predict (out ,type="coefficients",s=bestlam )[1:19,]
kable(round(rigge.coef[rigge.coef>0.0001 | rigge.coef<0 ],6),align = 'c',  booktabs = TRUE) %>%
  kable_styling(font_size = 15)
```
...
Por medio de validación cruzada obtendremos el mejor lambda y realizaremos la predicciones con este:
...
kable(sqrt(mean(( lasso.pred -y.test)^2)),align = 'c',  booktabs = TRUE) %>%
  kable_styling(font_size = 15)
```

Las variables obpenidas con LASSO son:

```{r}
out=glmnet (x,y,alpha =1, lambda =grid)
lasso.coef=predict(out, type ="coefficients",s=bestlam )[1:19,]
kable(lasso.coef[lasso.coef!=0],align = 'c',  booktabs = TRUE) %>%
  kable_styling(font_size = 15)
```
...
Los resultados obtenidos por ridge y lasso son:

```{r}
# ride: me deja muchas variables
kable(round(rigge.coef[rigge.coef>0.005 | rigge.coef < 0],4),align = 'c',  booktabs = TRUE) %>%
  kable_styling(font_size = 15)
...
Paara la division en entrenamiento y prueba, considerando la cantidad de datos que se tienen, se opta por hacer un particionamiento para modelar, en lugar de utilizar validación cruzada:
...
Modelo lienal con las variables previamente halladas por leaps, con sus respectivos RMSE:
...
kable(summary(mod_leaps)$coef,  booktabs = TRUE) %>%
  kable_styling(font_size = 15)%>% 
  row_spec(0, color = 'white', background = 'black')

pred_leaps <- predict(mod_leaps, newdata = testing)
rmse_leaps <- sqrt(mean((pred_leaps-testing$n)^2)); 
```
...
kable(summary(mod_ridge)$coef,  booktabs = TRUE) %>%
  kable_styling(font_size = 15)
pred_ridge <- predict(mod_ridge, newdata = testing)
rmse_ridge <- sqrt(mean((pred_ridge-testing$n)^2))
```
...
kable(summary(mod_lasso)$coef,  booktabs = TRUE) %>%
  kable_styling(font_size = 15)
pred_lasso <- predict(mod_lasso, newdata = testing)
rmse_lasso <- sqrt(mean((pred_lasso-testing$n)^2))
```

Podemos concluir que el mejor modelo según el RMSE de prueba es el construido con los métodos de forward, backward y stepwyse, pero su diferencia es por poco.

Ahora será necesaria la validación de supuestos:

Para modelo con leaps: ![](reiduales%20both.png)
...
Para moelo con lasso: ![](residuals%20lasso.png)
...
Para modelo con ridge: ![](residuals%20rich.png)
...
Concluimos que los modelos tienen serios problemas con el cumplimiento de los supuestos de homocedasticidad y de normalidad de los errores. Se intentaron plantear algunas transformaciones, pero no se observa un patrón claro en los gráficos de los errores para aplicar determinada transformación como una forma cuadrática o exponencial etc. y al aplicarlos no mostraron una mejora significativa en su RMSE.

Hallamos box-cox con el propósito de lograr cumplir el supuesto de normalidad, pero, no puede ser hallada pues los valores de la variable respuesta deben ser positivos y hay muchos ceros.
...
Ahora se realiza el ajuste del modelo con un valor cp(parámetro de complejidad) pequeño, que nos ayudará a encontrar una profundidad adecuada para obtener el mejor árbol, es decir que tenga menor error de entrenamiento. Una cualidad de los árboles de regresión es que no hay que realizar una previa selección de variables, él tomara en su raíz las que considera más importantes, y la siguiente salida de R nos mostrará los resultados de las variables seleccionadas:
...
rmse_tree
RMSE20
df_modelos <- data.frame(lm_lasso = rmse_lasso, lm_ridge = rmse_ridge, lm_leaps = rmse_leaps,
                         tree = rmse_tree, knn = RMSE20, row.names = 'RMSE')

kable(df_modelos,  booktabs = TRUE) %>%
  kable_styling(font_size = 15)%>% 
  row_spec(0, color = 'white', background = 'black')

```

numéricamente los valores son muy similares, pero favoreciendo al árbol, pero podríamos quedarnos con el modelo de árboles de decisión pues no debe cumplir con supuestos, a diferencia de los demás que tienen serios problemas con sus supuestos.
...
* El mejor modelo resulto siendo KNN con K=20 con un RMSE de 0.875389 seguido por el segundo mejor con un RMSE de 1.047902 para los arboles de regresión.
